...
void test_square_area_centroid() {
  ...
  assert(isclose(polygon_area((list_t *)sq), 4));
  assert(vec_isclose(polygon_centroid((list_t *)sq), VEC_ZERO));
  vec_list_free(sq);
}

void test_square_translate() {
  vec_list_t *sq = make_square();
  polygon_translate((list_t *)sq, (vector_t){2, 3});
  ...
  assert(isclose(polygon_area((list_t *)sq), 4));
  assert(vec_isclose(polygon_centroid((list_t *)sq), (vector_t){2, 3}));
  vec_list_free(sq);
}

void test_square_rotate() {
  vec_list_t *sq = make_square();
  polygon_rotate((list_t *)sq, 0.25 * M_PI, VEC_ZERO);
...
void test_triangle_area_centroid() {
  ...
  assert(isclose(polygon_area((list_t *)tri), 6));
  assert(
      vec_isclose(polygon_centroid((list_t *)tri), (vector_t){8.0 / 3.0, 1}));
  vec_list_free(tri);
}

void test_triangle_translate() {
  vec_list_t *tri = make_triangle();
  polygon_translate((list_t *)tri, (vector_t){-4, -3});
  ...
  assert(isclose(polygon_area((list_t *)tri), 6));
  assert(
      vec_isclose(polygon_centroid((list_t *)tri), (vector_t){-4.0 / 3.0, -2}));
  vec_list_free(tri);
}
...
void test_triangle_rotate() {
  ...
  polygon_rotate((list_t *)tri, -acos(4.0 / 5.0), (vector_t){4, 3});
  ...
  assert(isclose(polygon_area((list_t *)tri), 6));
  assert(vec_isclose(polygon_centroid((list_t *)tri),
                     (vector_t){26.0 / 15.0, 2.2}));
...
void test_circ_area_centroid() {
  ...
  assert(isclose(polygon_area((list_t *)c), CIRC_AREA));
  assert(vec_isclose(polygon_centroid((list_t *)c), VEC_ZERO));
  vec_list_free(c);
}

void test_circ_translate() {
  vec_list_t *c = make_big_circ();
  polygon_translate((list_t *)c, (vector_t){100, 200});

  for (size_t i = 0; i < CIRC_NPOINTS; i++) {
    double angle = 2 * M_PI * i / CIRC_NPOINTS;
    assert(vec_isclose(*vec_list_get(c, i),
                       (vector_t){100 + cos(angle), 200 + sin(angle)}));
  }
  assert(isclose(polygon_area((list_t *)c), CIRC_AREA));
  assert(vec_isclose(polygon_centroid((list_t *)c), (vector_t){100, 200}));
...
void test_circ_rotate() {
  ...
  vec_list_t *c = make_big_circ();
  polygon_rotate((list_t *)c, ROT_ANGLE, VEC_ZERO);

  for (size_t i = 0; i < CIRC_NPOINTS; i++) {
    double angle = 2 * M_PI * i / CIRC_NPOINTS;
    assert(
        vec_isclose(*vec_list_get(c, i), (vector_t){cos(angle + ROT_ANGLE),
                                                    sin(angle + ROT_ANGLE)}));
  }
  assert(isclose(polygon_area((list_t *)c), CIRC_AREA));
  assert(vec_isclose(polygon_centroid((list_t *)c), VEC_ZERO));
...
void test_weird_area_centroid() {
  ...
  assert(isclose(polygon_area((list_t *)w), 23));
  assert(vec_isclose(polygon_centroid((list_t *)w),
...
void test_weird_translate() {
  vec_list_t *w = make_weird();
  polygon_translate((list_t *)w, (vector_t){-10, -20});
...
void test_weird_rotate() {
  ...
  polygon_rotate((list_t *)w, M_PI / 2, (vector_t){0, 2});
