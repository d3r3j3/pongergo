...
void test_list_size0() {
  poly_list_t *pl = poly_list_init(0);
  assert(poly_list_size(pl) == 0);
  poly_list_free(pl);
}

void test_list_size1() {
  poly_list_t *pl = poly_list_init(1);
  assert(poly_list_size(pl) == 0);
  // Add
  vec_list_t *vl = create_vec_list(1, 0, 0);
  poly_list_add(pl, vl);
  assert(poly_list_size(pl) == 1);
  // Remove
  assert(poly_list_remove(pl) == vl);
  vec_list_free(vl);
  assert(poly_list_size(pl) == 0);
  // Add again
  ...
  assert(poly_list_size(pl) == 1);
  // Modify
  vec_list_get(poly_list_get(pl, 0), 0)->x = 1;
  vec_list_get(poly_list_get(pl, 0), 0)->y = 2;
  vector_t va = *vec_list_get(poly_list_get(pl, 0), 0);
  assert(vec_equal(va, (vector_t){1, 2}));
  free(vec_list_get(poly_list_get(pl, 0), 0));
  vec_list_array_free(poly_list_get(pl, 0));
  poly_list_array_free(pl);
}
#define LARGE_SIZE 10000

void test_list_large_get_set() {
  poly_list_t *pl = poly_list_init(LARGE_SIZE);
  // Add to capacity
  ...
  // Check
  for (size_t i = 0; i < LARGE_SIZE; i++) {
    size_t curr_size = vec_list_size(poly_list_get(pl, i));
    assert(curr_size == i);
    vec_list_t *vl = poly_list_get(pl, i);
  ...
  for (size_t i = 0; i < LARGE_SIZE; i++) {
    vec_list_free(poly_list_get(pl, i));
  }
